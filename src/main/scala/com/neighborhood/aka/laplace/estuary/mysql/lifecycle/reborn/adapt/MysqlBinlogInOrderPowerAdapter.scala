...
import com.neighborhood.aka.laplace.estuary.mysql.lifecycle.reborn.adapt.MysqlBinlogInOrderPowerAdapterCommand._
...
/**
  ...
  */
abstract class MysqlBinlogInOrderPowerAdapter(
  ...
  override def receive: Receive = {
    case FetcherMessage(MysqlBinlogInOrderPowerAdapterUpdateCost(cost)) => updateFetchTimeByTimeCost(cost)
    case BatcherMessage(MysqlBinlogInOrderPowerAdapterUpdateCost(cost)) => updateBatchTimeByTimeCost(cost)
    case SinkerMessage(MysqlBinlogInOrderPowerAdapterUpdateCost(cost)) => updateSinkTimeByTimeCost(cost)
    case SyncControllerMessage(MysqlBinlogInOrderPowerAdapterControl)
    => control
    case SyncControllerMessage(MysqlBinlogInOrderPowerAdapterComputeCost)
    => computeCost
  }
  ...
  override def control: Unit = {
    ...
    val computeDelayDuration: Long = (diff, fetchCost, batchCost, sinkCost) match {
      ...
      //      case (w, _, _, _) if (w < 82000 * 1) => math.max(delayDuration * 10, 100000) //100ms
      //      case (w, _, _, _) if (w < 100000 * 1) => math.max(delayDuration, 7000) //7ms
      //      case (w, _, _, _) if (w < 120000 * 1) => math.max(delayDuration, 300000) //300ms
    ...
    if (lastDelayDuration != finalDelayDuration) { //如果跟上次的值不同，再做操作
      context.parent ! PowerAdapterMessage(MysqlBinlogInOrderPowerAdapterDelayFetch(finalDelayDuration)) //向syncController发送fetchDelay
...
object MysqlBinlogInOrderPowerAdapter {
  def buildMysqlBinlogInOrderPowerAdapterByName(taskManager: TaskManager, name: String): Props = name match {
    case _ => DefaultMysqlBinlogInOrderPowerAdapter.props(taskManager)
  }
}
