...
final class MysqlBinlogInOrderBatcherMysqlManager(
  ...
  override protected def initBatchers: Unit = {
    ...
    val paths: List[String] = (1 to batcherNum).map {
      index =>
        MysqlBinlogInOrderBatcherPrimaryKeyManager.buildMysqlBinlogInOrderBatcherPrimaryKeyManager(name, taskManager, taskManager.sinkerList(index - 1), index).withDispatcher("akka.batcher-dispatcher")
