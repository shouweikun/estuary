...
final class Oplog2HBaseMultiInstanceController(
  ...
  def collectChildInfo: Unit = {
    ...
    taskManager.fetchCost.set(Try(childTaskManagers.map(_.fetchCost.get()).sum / childTaskManagers.size).getOrElse(0))
    taskManager.batchCost.set(Try(childTaskManagers.map(_.batchCost.get()).sum / childTaskManagers.size).getOrElse(0))
    taskManager.sinkCost.set(Try(childTaskManagers.map(_.sinkCost.get()).sum / childTaskManagers.size).getOrElse(0))
    taskManager.sinkCountPerSecond.set(Try((sinkCount - lastSinkCount) / tem).getOrElse(0))
    taskManager.batchCountPerSecond.set(Try((batchCount - lastBatchCount) / tem).getOrElse(0))
    taskManager.fetchCountPerSecond.set(Try((fetchCount - lastFetchCount) / tem).getOrElse(0))
